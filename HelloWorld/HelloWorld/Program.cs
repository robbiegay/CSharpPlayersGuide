...
// use checked and unchecked to either check for (and throw an error) if there is overflow, or to not check (unchecked)
...
// https://csharpplayersguide.com/articles/operators-table

unsafe // added to .csproj: <AllowUnsafeBlocks>true</AllowUnsafeBlocks>
{
    int test = 4;
    int *address = &test;
...
while(keepRunning)
{
    ...
    if (runAgain != "y") keepRunning = false;
}
...
while (keepRunning)
{
    for (int i = 0; i < 3; i++)
    {
        ...
        Console.WriteLine($"Enter the number of duchyes for {kings[i]}:");
        int duchyes;
        int.TryParse(Console.ReadLine(), out duchyes);
        ...
        Console.WriteLine($"duchyes: {duchyes} x 3 points = {duchyes *= 3}");
        Console.WriteLine($"provinces: {provinces} x 6 points = {provinces *= 6}");
        Console.WriteLine($"Total = {estates + duchyes + provinces}");
    }
